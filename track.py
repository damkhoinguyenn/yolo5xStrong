...
def run(
        ...
        save_img=False,  # save results to *.jpg
    ...
    # Initialize an empty dictionary to store ID-color mappings
    id_color_list = {}
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if webcam:  # nr_sources >= 1
                ...
                p, im0, _ = path, im0s.copy(), getattr(dataset, 'frame', 0)
                p = Path(p)  # to Path
                txt_file_name = p.stem if source.endswith(VID_FORMATS) else p.parent.name
                save_path = str(save_dir / (p.name if source.endswith(VID_FORMATS) else p.parent.name))
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if save_vid:  # Add bbox to image
                            ...
                            if id not in id_color_list:
                                # Generate a random color for a new track ID
                                color = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
                                id_color_list[id] = color

                            color = id_color_list.get(id, (255, 255, 255))
            ...
            if save_img:
                img_filename = f'frame_{frame_idx + 1}.jpg'
                img_save_path = os.path.join(save_img_path, img_filename)
...
def parse_opt():
    ...
    parser.add_argument('--yolo-weights', nargs='+', type=str, default='weights/yolov5.pt', help='model.pt path(s)')
